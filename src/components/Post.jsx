...
const PostDetails = () => {
  ...
  const getPostById = async () => {
    try {
      ...
      // BUG FIX: setError ki jagah setErrors aayega
      setErrors('Failed to load post. Please try again');
    } finally {
      setLoading(false);
    }
  };
  ...
  // 🔥 DELETE POST FUNCTION WITH CONFIRMATION POPUP 🔥
  const handleDelete = async () => {
    // Ye line popup show karegi
    const confirmDelete = window.confirm("Are you sure you want to delete this post?");
    
    // Agar user 'Yes/OK' par click kare
    if (confirmDelete) {
      try {
        await API.delete(`/post/${id}`); // Aapki delete API ka endpoint yahan aayega
        alert("Post deleted successfully!");
        navigate("/"); // Delete hone ke baad home page par bhej do
      } catch (error) {
        console.error("Error deleting post", error);
        alert("Failed to delete post. Please try again.");
      }
    }
    // Agar user 'Cancel' par click karega toh kuch nahi hoga, yahi page rahega
  };
          ...
          <div className="flex items-center mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="font-semibold text-gray-900 dark:text-gray-400">@{post.author?.username}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                <button 
                  ...
                  className="flex items-center space-x-1 px-3 py-1 bg-blue-100 text-blue-600 hover:bg-blue-200 rounded-md transition-colors font-medium text-sm"
                >
                  <span>Edit</span>
                </button>
                ...
                <button 
                  ...
                  className="flex items-center space-x-1 px-3 py-1 bg-red-100 text-red-600 hover:bg-red-200 rounded-md transition-colors font-medium text-sm"
                >
                  <span>Delete</span>
                </button>
              </div>
